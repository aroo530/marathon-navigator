...
export default function ChallengesScreen() {
  ...
  const [weeks, setWeeks] = useState<
    { id: number; week_number: number; start_date: string; end_date: string }[]
  >([]);
  ...
  const fetchChallenges = async () => {
    ...
    try {
      ...
      const [
        { weekChallenges: weekly, generalChallenges: general },
        assignments,
      ] = await Promise.all([
        fetchMarathonChallenges(
          currentMarathonId,
      ...
      const assignmentMap = new Map(
        assignments.map((a) => [a.challenge_id, a]),
      );
    ...
    React.useCallback(() => {
      ...
      const init = async () => {
        ...
        if (canPickFamily && currentMarathonId) {
          ...
          if (fam && isActive) setActiveFamilyId(fam.id);
        }
      };
  ...
  const handleChallengePress = (challenge: Challenge) => {
    if (!canUserEditChallenge(challenge, userProfile?.role, userProfile?.id)) {
      ...
      showToast(
        ...
        t("challenges.lockedMessage", { owner }),
      );
      return;
    }
  ...
  const handleScoreSubmit = async (points: number, percentage?: number) => {
    console.log("Submitting score:", { points, percentage });
    ...
    try {
      ...
      showToast(
        "success",
        t("challenges.success"),
        t("challenges.scoreUpdated"),
      );
      ...
      <SafeAreaView style={styles.safeArea}>
        ...
        {canPickFamily && allFamilies.length > 0 && (
          <View style={styles.contextBar}>
            ...
            <View style={styles.weekBadgeContainer}>
              ...
              <View style={[styles.weekBadge, { backgroundColor: marathonTheme.tint }]}>
                <Text style={[styles.weekBadgeText, { color: marathonTheme.primary }]}>
                  Week{" "}
                  {weeks.find((w) => w.id === selectedWeekId)?.week_number ??
                    "—"}
                </Text>
              </View>
            </View>
            ...
            <TouchableOpacity
              ...
              style={[
                ...
                selectedWeekId === week.id && [
                  ...
                  {
                    backgroundColor: marathonTheme.primary,
                    borderColor: marathonTheme.primary,
                  },
                ],
              ]}
        ...
        <FlatList
          ...
          renderItem={({ item: section }) => (
            <View style={styles.section}>
              <Text
                style={[styles.sectionTitle, { color: marathonTheme.shade }]}
              >
                {section.title}
              </Text>
              {section.data.length === 0 ? (
                ...
                section.data.map((challenge) => {
                  const editable = canUserEditChallenge(
                    challenge,
                    userProfile?.role,
                    userProfile?.id,
                  );
                    ...
                    <View key={challenge.id}>
                      <View
                        style={[
                          ...
                          !editable && styles.challengeCardLocked,
                        ]}
                      >
                        ...
                        <View style={styles.challengeFooter}>
                          ...
                          <View style={styles.actionButtons}>
                            ...
                            <TouchableOpacity
                              style={[
                                styles.statusButton,
                                { backgroundColor: marathonTheme.tint },
                                !editable && styles.statusButtonLocked,
                              ]}
                            ...
                            >
                              ...
                              <Text
                                style={[
                                  ...
                                  { color: marathonTheme.primary },
                                  !editable && styles.statusButtonTextLocked,
                                ]}
                              >
                                {editable
        ...
        {!!selectedChallenge && (
          <ScoreInputModal
            ...
            totalFamilyMembers={
              canPickFamily
                ? (allFamilies.find((f) => f.id === activeFamilyId)?.member_count ?? 1)
